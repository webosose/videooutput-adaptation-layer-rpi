...
class DeviceCapability
{
    ...
    public:
        ...
        UINT16 h;
        UINT16 freq; // Hz
        operator VAL_VIDEO_SIZE_T()
        {
            VAL_VIDEO_SIZE_T ret;
            ret.w = w;
            ret.h = h;
            return ret;
        }
...
public:
    ...
    VAL_VIDEO_SIZE_T getMaxResolution() { return mMaxResolution; };
    VAL_VIDEO_SIZE_T getMinResolution() { return mMinResolution; };
    const std::set<std::string> &getPlaneNames() { return mPlaneNames; };
private:
    DeviceModeResolution mMaxResolution = {w : 1920, h : 1080, freq : 60};
...
560x1600#p1954437 there is a pixel clock limit which means the highest supported mode is 1920x1200 @60 Hz with
    reduced blanking.*/
    DeviceModeResolution mMinResolution = {w : 0, h : 0, freq : 60};
    /*note:in hdmi_safe mode w and h printed by tvservice is 640x480 which is listed the minimum res in
     * device-cap.json*/
