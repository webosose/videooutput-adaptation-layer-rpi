...
static const struct type_name encoder_type_names[] = {
    ...
    {DRM_MODE_ENCODER_LVDS, "LVDS"}, {DRM_MODE_ENCODER_TVDAC, "TVDAC"}, {DRM_MODE_ENCODER_VIRTUAL, "Virtual"},
...
static const struct type_name connector_status_names[] = {
    {DRM_MODE_CONNECTED, "connected"}, {DRM_MODE_DISCONNECTED, "disconnected"}, {DRM_MODE_UNKNOWNCONNECTION, "unknown"},
};
...
*Modes util
*/
static std::vector<std::string> mode_type_names{"builtin", "clock_c", "crtc_c", "preferred",
                                                "default", "userdef", "driver"};
static std::vector<std::string> mode_flag_names{"phsync",  "nhsync", "pvsync", "nvsync", "interlace",
...
std::ostream &operator<<(std::ostream &os, const DrmDisplayMode &dm)
{
    ...
    os << dm.mModeInfoPtr->name << " " << dm.mModeInfoPtr->vrefresh << " " << dm.mModeInfoPtr->hdisplay << " "
       << dm.mModeInfoPtr->hsync_start << " " << dm.mModeInfoPtr->hsync_end << " " << dm.mModeInfoPtr->htotal << " "
       << dm.mModeInfoPtr->vdisplay << " " << dm.mModeInfoPtr->vsync_start << " " << dm.mModeInfoPtr->vsync_end << " "
       << dm.mModeInfoPtr->vtotal << " " << dm.mModeInfoPtr->clock << " ";
...
std::ostream &operator<<(std::ostream &os, const DrmPlane &dm)
{
    ...
    os << ovr->plane_id << "\t" << ovr->crtc_id << "\t" << ovr->fb_id << "\t" << ovr->crtc_x << "\t" << ovr->crtc_y
       << "\t" << ovr->x << "\t" << ovr->y << "\t" << ovr->gamma_size << "x" << ovr->possible_crtcs;
